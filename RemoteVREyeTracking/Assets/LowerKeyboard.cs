...
public class LowerKeyboard : MonoBehaviour
{
    ...
    public void filledIn()
    {
        ...
        if (!(distraction.text.Trim().Length < 8) && !(distractionOnScreen.text.Trim().Length < 8) && !(difficultyStudy.text.Trim().Length == 0) && (distractionOnScreen.text.Equals(distraction.text)))
        {
            ...
            paypalNoDistract.GetComponent<UpperKeyboard>().password = distraction.text;
            paypalNoDistract.GetComponent<AltKeyboard>().password = distraction.text;
            email = difficultyStudy.text;
            paypalNoDistract.GetComponent<UpperKeyboard>().email = difficultyStudy.text;
